...
<xsl:stylesheet
      ...
      <xsl:choose>		
		<!-- Embed video row -->
		  <xsl:when test="$clause = 1">
			<div style="width:640px;max-height:480px;margin:auto;display:block;">
			<xsl:value-of select="dim:field[@element='url'][@qualifier='youtubeurl'][1]/node()" disable-output-escaping="yes"/>
			</div>
			<xsl:call-template name="itemSummaryView-DIM-fields">
          ...
          <!-- Author(s) row -->
          <xsl:when test="$clause = 3 and (dim:field[@element='contributor'][@qualifier='author' and descendant::text()] or dim:field[@element='creator' and descendant::text()] or dim:field[@element='contributor' and descendant::text()])">
          ...
          <xsl:when test="$clause = 4 and (dim:field[@element='identifier' and @qualifier='uri' and descendant::text()])">
				...
				<div class="social-buttons-wrapper">
					<div >					
						<a class="facebook-share" target="_blank" rel='nofollow'>
							<xsl:attribute name="href">
								<xsl:value-of select="concat('http://www.facebook.com/sharer.php?u=', dim:field[@qualifier='uri'])"/>
							</xsl:attribute>
					...
					<div >
						...
						<div class="g-plus" data-action="share"><span class="hidden">google</span></div>
					</div>
				</div>
              <xsl:call-template name="itemSummaryView-DIM-fields">
                <xsl:with-param name="clause" select="($clause + 1)"/>
    ...
    <xsl:template match="mets:file">
		...
		<xsl:call-template name="view-media"/>
    ...
    <xsl:template name="view-media">
		<xsl:if test="(@MIMETYPE='image/jpeg')">             
			<img style="max-width:600px;max-height:600px;margin:auto;display:inline-block;">
			<xsl:attribute name="src">
				<xsl:value-of select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>					
			</xsl:attribute>
		</img>
		</xsl:if>
		<xsl:if test="(@MIMETYPE='image/gif')">             
			<img style="max-width:600px;max-height:600px;margin:auto;display:inline-block;">
			<xsl:attribute name="src">
				<xsl:value-of select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>					
			</xsl:attribute>
		...
		<xsl:if test="(@MIMETYPE='image/png')">             
			<img style="max-width:600px;max-height:600px;margin:auto;display:inline-block;">
			<xsl:attribute name="src">
				<xsl:value-of select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>					
			</xsl:attribute>
		...
		<xsl:if test="(@MIMETYPE='video/mp4')">             
			<video controls="controls" style="width:640px;max-height:480px;margin:auto;display:inline-block;">				
					<xsl:attribute name="src">
						<xsl:value-of select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>
					</xsl:attribute>				
		...
		<xsl:if test="(@MIMETYPE='video/webm')">             
			<video controls="controls" style="width:640px;max-height:480px;margin:auto;display:block;">				
					<xsl:attribute name="src">
						<xsl:value-of select="mets:FLocat[@LOCTYPE='URL']/@xlink:href"/>
					</xsl:attribute>				
				<p>Your browser does not support the video tag.</p>				
